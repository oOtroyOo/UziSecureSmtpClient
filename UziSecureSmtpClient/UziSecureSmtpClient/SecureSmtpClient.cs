    ...
    public class SecureSmtpClient : IDisposable
    {
                ...
                )
        {
            try
            {
                ...
                // send from
                SendDataLine(string.Format("From: {0} <{1}>", SecureSmtpPart.Utf8Base64Encode(Message.From.DisplayName), Message.From.Address));
        ...
        private void OpenConnection()
        {
            ...
            SmtpReplyCode ReplyCode = GetReply(out string msg);
        ...
        private void SendRecipientAddress
                (
                List<SecureSmtpMailAddress> AddressList
                )
        {
            // send one address at a time
            foreach (SecureSmtpMailAddress MA in AddressList)
            {
                ...
                )
        {
            ...
            foreach (SecureSmtpMailAddress MA in AddressList)
            {
                if (List.Length == 0)
                {
                    ...
                    List.AppendFormat("{0}: {1} <{2}>", Type, SecureSmtpPart.Utf8Base64Encode(MA.DisplayName), MA.Address);
                }
                else
                {
                    // append all others
                    List.AppendFormat(",\r\n {0} <{1}>", SecureSmtpPart.Utf8Base64Encode(MA.DisplayName), MA.Address);
                }
            }
        ...
        */
        ...
        ////////////////////////////////////////////////////////////////////
        private SmtpReplyCode GetReply(out string Str)
        {
            // reset reply
            ReplyStrings.Clear();
